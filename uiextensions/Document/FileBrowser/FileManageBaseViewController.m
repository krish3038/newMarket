...
- (void)refreshInterface {
    _searchBar.placeholder = FSLocalizedString(@"kSearchPlaceholder");
}

#pragma mark - Private API

- (BOOL)checkDiskFile:(int)currentSelectRow {
    FileItem *selectedFile = [arrayFile objectAtIndex:currentSelectRow];
    ...
    FileItem *diskFile = [[FileItem alloc] initWithPath:selectedFile.path modifiedDate:nil isFavorite:nil];
    if (!diskFile.isValidPDF) //cannot find the file in this path
    {
        AlertView *alertView = [[AlertView alloc] initWithTitle:@"kWarning" message:[NSString stringWithFormat:FSLocalizedString(@"kNoFileInDisk"), selectedFile.fileName] buttonClickHandler:nil cancelButtonTitle:nil otherButtonTitles:@"kOK", nil];
        [alertView show];
...
- (void)setNavigationBarTitleForFolderName {
    if ([[self.currentFolder lastPathComponent] isEqualToString:@"Documents"]) {
        self.title = FSLocalizedString(@"kDocuments");
    } else {
        {
...
- (NSArray *)getFileObjectFromFileNames:(NSArray *)fileNames {
    ...
    for (NSString *file in fileNames) {
        FileItem *fileObject = [[FileItem alloc] initWithPath:file modifiedDate:nil isFavorite:nil];
...
- (void)thumbnailScrollView:(ThumbnailScrollView *)scrollView didTapOnCellAtIndex:(NSInteger)index {
    self.fileItem = nil;
    FileItem *openingFile = [arrayFile objectAtIndex:index];
    if (!openingFile.isFolder) {
        if (!_thumbnailFileList.editing && !openingFile.isOpen) //open selected file if it's not opened yet
        {
            if ([self checkDiskFile:(int) index]) {
                ...
                FileItem *openedFile = nil;
                for (FileItem *obj in arrayFile) {
                    if (obj.isOpen) {
                        openedFile = obj;
                        break;
                    }
                ...
                if ([self.delegate conformsToProtocol:@protocol(FileDelegate)] && [self.delegate respondsToSelector:@selector(onItemCliked:)]) {
