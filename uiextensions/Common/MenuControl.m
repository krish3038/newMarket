...
- (void)showMenu {
    ...
    if (annot) {
        ...
        if ((dvRect.origin.x + dvRect.size.width) <= 0 || dvRect.origin.y + dvRect.size.height <= 0 || dvRect.origin.x > CGRectGetWidth(_pdfViewCtrl.bounds) || dvRect.origin.y > CGRectGetHeight(_pdfViewCtrl.bounds)) {
            return;
        }
    }
