...
- (void)viewDidLoad {
    ...
    if (self.navigationItem.titleView != nil) {
        ...
        if (titleLabel) {
            titleLabel.text = FSLocalizedString(@"kPermission");
        }
    }
    ...
    self.tableView.autoresizingMask = UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleHeight | UIViewAutoresizingFlexibleRightMargin | UIViewAutoresizingFlexibleTopMargin | UIViewAutoresizingFlexibleBottomMargin | UIViewAutoresizingFlexibleLeftMargin;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    case 0:
        permission.text = FSLocalizedString(@"kPermissionPrint");
        if (self.allowPrint) {
            allow.text = FSLocalizedString(@"kEnable");
        } else {
            allow.text = FSLocalizedString(@"kDisable");
        }
        break;
    case 1:
        permission.text = FSLocalizedString(@"kRMSFILLFORM");
        if (self.allowFillForm) {
            allow.text = FSLocalizedString(@"kEnable");
        } else {
            allow.text = FSLocalizedString(@"kDisable");
        }
        break;
    case 2:
        permission.text = FSLocalizedString(@"kRMSASSEMBLE");
        if (self.allowAssemble) {
            allow.text = FSLocalizedString(@"kEnable");
        } else {
            allow.text = FSLocalizedString(@"kDisable");
        }
        break;
    case 3:
        permission.text = FSLocalizedString(@"kRMSANNOTATE");
        if (self.allowAnnotate) {
            allow.text = FSLocalizedString(@"kEnable");
        } else {
            allow.text = FSLocalizedString(@"kDisable");
        }
        break;
    case 4:
        permission.text = FSLocalizedString(@"kRMSEDIT");
        if (self.allowEdit) {
            allow.text = FSLocalizedString(@"kEnable");
        } else {
            allow.text = FSLocalizedString(@"kDisable");
        ...
        break;
    case 5:
        permission.text = FSLocalizedString(@"kRMSEXTRACTACCESS");
        if (self.allowExtractAccess) {
            allow.text = FSLocalizedString(@"kEnable");
        } else {
            allow.text = FSLocalizedString(@"kDisable");
        }
        break;
    case 6:
        permission.text = FSLocalizedString(@"kRMSEXTRACT");
