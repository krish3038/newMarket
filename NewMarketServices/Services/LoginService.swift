...
final class LoginService {
    ...
    func login(credentials: LoginReq, completion: @escaping ( _ result:Any? ,_ error:Error?) ->Void){
        let networkManager = NetworkService.shared
        
        let serviceUrl = ServiceUrls.hostUrl + ServiceUrls.LoginServiceApi.loginUrl
    ...
    func parseResponseData(data:Any) -> Any? {
        ...
        do {
            ...
            return try  [LoginResponse].decode(data: data as! Data)
        }
        catch {
