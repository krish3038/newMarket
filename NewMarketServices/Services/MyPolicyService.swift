...
final class MyPolicyService {
    
    func getPolicy(credentials: LoginReq, completion: @escaping ( _ result:Any? ,_ error:Error?) ->Void){
        let networkManager = NetworkService.shared
        
        let serviceUrl = ServiceUrls.hostUrl + ServiceUrls.MypolicyServiceApi.myPolicyUrl
    ...
    func parseResponseData(data:Any) -> Any? {
        ...
        do {
            ...
            return try  MyPolicyResponse.decode(data: data as! Data)
        }
        catch {
