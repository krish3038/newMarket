//
//  ClaimHousekeepCheckService.swift
//  NewMarketServices
//
...
final class ClaimHousekeepCheckService {
    
    func getHouseKeepCheckData(credentials: LoginReq, claimNo : String , completion: @escaping ( _ result:Any? ,_ error:Error?) ->Void){
        let networkManager = NetworkService.shared
        
        let serviceUrl = ServiceUrls.hostUrl + ServiceUrls.ClaimHousekeepCheckServiceApi.claimHousekeepCheckUrl + claimNo
    ...
    func parseResponseData(data:Any) -> Any? {
        ...
        do {
            ...
            return try  ClaimHousekeepCheckResponse.decode(data: data as! Data)
        }
        catch {
