...
final class ClaimSettlementService {
    ...
    func getClaimSettlementData(credentials: LoginReq, claimNo : String , completion: @escaping ( _ result:Any? ,_ error:Error?) ->Void){
        let networkManager = NetworkService.shared
        
        let serviceUrl = ServiceUrls.hostUrl + ServiceUrls.ClaimSettlementServiceApi.claimSettlementUrl + claimNo
    ...
    func parseResponseData(data:Any) -> Any? {
        ...
        do {
            ...
            return try  ClaimSettlementResponse.decode(data: data as! Data)
        }
        catch {
