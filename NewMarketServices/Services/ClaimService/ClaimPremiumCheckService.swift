//
//  ClaimPremiumCheckService.swift
//  NewMarketServices
//
...
final class ClaimPremiumCheckService {
    
    func getPremiumCheckData(credentials: LoginReq, claimNo : String , completion: @escaping ( _ result:Any? ,_ error:Error?) ->Void){
        let networkManager = NetworkService.shared
        
        let serviceUrl = ServiceUrls.hostUrl + ServiceUrls.ClaimPremiumCheckServiceApi.claimPremiumCheckUrl + claimNo
    ...
    func parseResponseData(data:Any) -> Any? {
        ...
        do {
            ...
            return try  ClaimPremiumCheckResponse.decode(data: data as! Data)
        }
        catch {
