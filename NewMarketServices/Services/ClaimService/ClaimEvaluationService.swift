...
final class ClaimEvaluationService {
    
    func getClaimEvaluationData(credentials: LoginReq, claimNo : String , completion: @escaping ( _ result:Any? ,_ error:Error?) ->Void){
        let networkManager = NetworkService.shared
        
        let serviceUrl = ServiceUrls.hostUrl + ServiceUrls.ClaimEvaluationServiceApi.claimEvaluationUrl + claimNo
    ...
    func parseResponseData(data:Any) -> Any? {
        ...
        do {
            ...
            return try  ClaimEvaluationResponse.decode(data: data as! Data)
        }
        catch {
