...
final class PolicyPdfService: NSObject {
    
    func getClaimPdf(credentials: LoginReq, claimNo : String , completion: @escaping ( _ result:Any? ,_ error:Error?) ->Void){
        let networkManager = NetworkService.shared
        
        let serviceUrl = ServiceUrls.hostUrl + ServiceUrls.PolicyPdfServiceApi.pdfServiceUrl + claimNo
