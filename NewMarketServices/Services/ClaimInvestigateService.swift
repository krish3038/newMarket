...
final class ClaimInvestigateService {
    
    func getClaimInvestigateRecord(credentials: LoginReq, claimNo : String , completion: @escaping ( _ result:Any? ,_ error:Error?) ->Void){
        let networkManager = NetworkService.shared
        
        let serviceUrl = ServiceUrls.hostUrl + ServiceUrls.ClaimInvestigateServiceApi.claimInvestigateUrl + claimNo
    ...
    func parseResponseData(data:Any) -> Any? {
        ...
        do {
            ...
            return try  ClaimInvestigateResponse.decode(data: data as! Data)
        }
        catch {
